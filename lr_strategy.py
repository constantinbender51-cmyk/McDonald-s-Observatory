...
for sma_window in range(1, 11):                       # 1 … 10
    pred25_sma = pd.Series(pred25).rolling(sma_window).mean().values
    pred25_deriv = np.full_like(pred25_sma, np.nan)
    pred25_deriv[1:] = pred25_sma[1:] - pred25_sma[:-1]

    for deriv_thresh in np.arange(0.1, 5.05, 0.1):    # 0.1 … 5.0
        for stop_pct in np.arange(0.2, 4.6, 0.2):     # 0.2 … 4.5

            capital = 1000.0
            lev     = 4.0
            pos     = 0
            entry_i = 0
            entry_pred25_abs = 0.0
            max_cap = capital
            worst_dd = 0.0
            trades = 0
            equity_curve = []
            ...
            for i in range(len(pct1d)):
                ...
                if pos != 0:
                    ...
                    if realised <= -stop_pct * entry_pred25_abs:
                        desired = 0
